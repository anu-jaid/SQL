--PROJECT 7 Delete duplicate Emails

CREATE TABLE Emails ( id INT PRIMARY KEY, NAME VARCHAR(50), email VARCHAR(100), phone VARCHAR(15));
...
DELETE FROM Emails
WHERE id NOT IN ( SELECT MIN(id) FROM (SELECT id, email FROM Emails ) AS subquery GROUP BY email);
